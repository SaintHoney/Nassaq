...
# --- Timetable Versions List API (New) ---
@api_router.get("/smart-scheduling/timetable/versions")
async def get_timetable_versions(
    current_user: dict = Depends(get_current_user),
    x_school_context: Optional[str] = Header(None)
...
# --- Active Timetable Sessions API (New) ---
@api_router.get("/smart-scheduling/timetable/active/sessions")
async def get_active_timetable_sessions(
    class_id: Optional[str] = None,
    teacher_id: Optional[str] = None,
...
# --- Generate Smart Timetable API (Wrapper) ---
...
async def generate_smart_timetable(
    ...
    """
    توليد الجدول الذكي بالذكاء الاصطناعي
    Generate smart timetable using AI
    """
    body = await request.json()
    school_id = body.get("school_id") or x_school_context or current_user.get("school_id")
    ...
    try:
        ...
        return {
            "success": True,
            "timetable_id": result.timetable_id if hasattr(result, 'timetable_id') else result.get("timetable_id"),
            "message_ar": "تم توليد الجدول بنجاح",
            "message_en": "Timetable generated successfully"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"فشل في توليد الجدول: {str(e)}")


# --- Publish Timetable API (Wrapper) ---
@api_router.post("/timetable/publish/{version_id}")
async def publish_timetable_version(
    ...
    if not success:
        raise HTTPException(
            status_code=400,
            detail="لا يمكن نشر الجدول - قد يوجد تعارضات أو أخطاء"
        )
    
    return {
        "success": True,
        "version_id": version_id,
        "message_ar": "تم نشر النسخة بنجاح",
        "message_en": "Version published successfully"
    }
