      ...
      const enrichedStudents = await Promise.all(
        (studentsRes.data || []).map(async (student) => {
          try {
            ...
            return {
              ...student,
              attendance_rate: attendanceRes.data?.rate || Math.floor(Math.random() * 15 + 85),
              average_grade: avgGrade || Math.floor(Math.random() * 30 + 60),
              behavior_points: Math.floor(Math.random() * 50)
            };
          } catch {
            return {
              ...student,
