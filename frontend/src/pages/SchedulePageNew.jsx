    ...
    <Sidebar>
      <div className="min-h-screen bg-gradient-to-br from-background via-muted/10 to-background" data-testid="schedule-page-new">
        ...
        <main className="p-6 space-y-6 max-w-full">
          ...
          {/* ============================================ */}
          {/* عرض الجدول الكامل في نفس الصفحة */}
          {/* ============================================ */}
          {showFullTimetable && selectedSmartTimetable && (
            <Card className="border-2 border-brand-navy/20 overflow-hidden mb-6" data-testid="full-timetable-view">
              <CardHeader className="bg-gradient-to-r from-brand-navy to-brand-turquoise text-white">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Calendar className="h-6 w-6" />
                    <div>
                      <CardTitle className="text-white text-xl">الجدول الدراسي الكامل</CardTitle>
                      <CardDescription className="text-white/80">
                        عرض شامل لجميع الحصص المجدولة
                      </CardDescription>
                    </div>
                  </div>
                </div>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex flex-wrap items-center gap-4 mb-6 p-4 bg-muted/50 rounded-xl">
                  {/* اختيار طريقة العرض */}
                  <div className="flex items-center gap-2">
                    <Tabs value={timetableViewMode} onValueChange={setTimetableViewMode} className="w-auto">
                      <TabsList className="h-9">
                        <TabsTrigger value="class" className="text-sm px-4 data-[state=active]:bg-brand-navy data-[state=active]:text-white">
                          <GraduationCap className="h-4 w-4 ml-1" />
                          حسب الفصل
                        </TabsTrigger>
                        <TabsTrigger value="teacher" className="text-sm px-4 data-[state=active]:bg-brand-navy data-[state=active]:text-white">
                          <User className="h-4 w-4 ml-1" />
                          حسب المعلم
                        </TabsTrigger>
                        <TabsTrigger value="all" className="text-sm px-4 data-[state=active]:bg-brand-navy data-[state=active]:text-white">
                  ...
                  {timetableViewMode === 'teacher' && (
                    <Select value={selectedViewTeacher} onValueChange={setSelectedViewTeacher}>
                      <SelectTrigger className="w-[200px]" data-testid="select-view-teacher">
                        <SelectValue placeholder="اختر المعلم" />
                      </SelectTrigger>
                      <SelectContent>
                        {teachers.map(t => (
                  ...
                  <div className="mr-auto flex items-center gap-4">
                    ...
                    <Badge variant="outline" className="text-sm py-1 px-3 bg-green-50 border-green-300 text-green-700">
                ...
                <div className="overflow-x-auto rounded-xl border">
                  <table className="w-full border-collapse min-w-[900px]" dir="rtl">
                    <thead>
                      <tr className="bg-gradient-to-r from-brand-navy to-brand-turquoise text-white">
                        <th className="p-3 text-center font-medium border-b border-e border-white/20 w-[120px]">
                          الحصة / اليوم
                        </th>
                        {DAYS.map(day => (
                          <th key={day.key} className="p-3 text-center font-medium border-b border-e border-white/20">
                      ...
                      {timeSlots.map((slot, idx) => {
                        ...
                        return (
                          <tr key={slot.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-muted/20'}>
                            <td className="p-2 border border-border bg-muted/30 text-center">
                            ...
                            {DAYS.map(day => {
                                ...
                                return (
                                  <td key={`${day.key}-${slot.id}`} className="p-1 border border-border">
                                    <div className={`p-2 rounded-lg text-center ${slot.type === 'prayer' ? 'bg-emerald-50' : 'bg-amber-50'}`}>
                                      {slot.type === 'prayer' ? (
                                        <Moon className="h-4 w-4 mx-auto text-emerald-600" />
                                      ) : (
                                        <Coffee className="h-4 w-4 mx-auto text-amber-600" />
                                      )}
                                      <p className={`text-xs font-medium mt-1 ${slot.type === 'prayer' ? 'text-emerald-700' : 'text-amber-700'}`}>
                              ...
                              // تصفية الجلسات حسب طريقة العرض
                              let filteredSessions = sessions.filter(s => {
                                const dayMatch = s.day_of_week === day.key || s.day === day.key;
                                const slotMatch = s.time_slot_id === slot.id || s.period_number === slot.slot_number;
                                return dayMatch && slotMatch;
                              });
                              
                              if (timetableViewMode === 'class' && selectedViewClass) {
                                filteredSessions = filteredSessions.filter(s => s.class_id === selectedViewClass);
                              } else if (timetableViewMode === 'teacher' && selectedViewTeacher) {
                                filteredSessions = filteredSessions.filter(s => s.teacher_id === selectedViewTeacher);
                              }
                                  ...
                                  <div className="space-y-1 max-h-[120px] overflow-y-auto">
                                    {filteredSessions.length > 0 ? filteredSessions.map((session, i) => {
                                      ...
                                      return (
                                        <div 
                                          key={i} 
                                          className={`p-2 rounded-lg ${colors.bg} ${colors.border} border cursor-pointer hover:shadow-md transition-all`}
                                          onClick={() => {
                                        ...
                                        >
                                          <p className={`font-bold text-xs ${colors.text} truncate`}>
                                            {session.subject_name?.split('/')[0]?.substring(0, 15) || 'مادة'}
                                          </p>
                                          {timetableViewMode !== 'teacher' && (
                                          ...
                                          {timetableViewMode !== 'class' && (
                                            <Badge variant="secondary" className="text-[9px] mt-1">
                                              {session.class_name?.replace('الصف ', '')?.substring(0, 12) || 'فصل'}
                                            </Badge>
                                          )}
                                        </div>
                                      );
                                    }) : (
                                      <div className="p-2 text-center text-muted-foreground/40 text-xs min-h-[50px] flex items-center justify-center">
                                        -
                                      </div>
                                    )}
                                  </div>
