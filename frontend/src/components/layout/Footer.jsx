...
export const Footer = () => {
  ...
  const footerLinks = {
    product: [
      ...
      { label: isRTL ? 'التكاملات' : 'Integrations', href: '/#integrations' },
    ],
    company: [
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
            ...
            <ul className="space-y-3">
              {footerLinks.product.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-white/70 hover:text-brand-turquoise transition-colors text-sm"
                  >
                    {link.label}
                  </a>
                </li>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-white/50 text-sm">
            {isRTL
              ? '© 2024 نَسَّق. جميع الحقوق محفوظة.'
              : '© 2024 NASSAQ. All rights reserved.'}
          </p>
